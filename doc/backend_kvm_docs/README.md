...
1. **[Architecture and Lifecycle](architecture_and_lifecycle.md)**
   ...
   - Host prerequisites and required dependencies (`virsh`, `libvirtd`, `qemu`, etc.).
...
2. **[Config Generator Modules Deep Dive](config_generator.md)**
   ...
   - Submodule reference:
     - `cpu.rs`: architecture detection (`x86_64` / `aarch64`), KVM vs. QEMU TCG fallback, vCPU allocation hierarchy, and `<cputune>` generation.
     ...
     - `disk.rs`: virtual disk concepts (file-backed and LVM-backed storage).
...
3. **[Container Boot Config Reference (`/boot/config.json`)](config_json_reference.md)**
   ...
   - Step-by-step tutorial on building a container image with Docker.
